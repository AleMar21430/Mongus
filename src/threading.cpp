...
void Database_Thread::processWork(const Mongo_Query& i_work) {
	...
	emit result(i_work.request_id, "");
}

void Database_Thread::queueWork(const Mongo_Query& i_work) {
	QMutexLocker locker(&semaphore);
	work_queue.enqueue(i_work);
	condition.wakeOne();
}

void Database_Thread::cancelWork() {
	QMutexLocker locker(&semaphore);
	work_queue.clear();
	condition.wakeOne();
}